...
# -- Imports --
import tkinter as tk, os, CoolProp.CoolProp as CP, numpy as np
...
def func_create_step(step_number = None):
    ...
    ModuleStepGeometry.create_step(root, tab3, step_number)
...
def func_create_seg(seg_number = None):
    ...
    ModuleSegmentGeometry.create_seg(root, tab3, seg_number)
...
#Warning
warning = ttk.Label(tab2, text='IP = Injector Plate, numbered "-1"').grid(column=0, row=10, padx=5, pady=5, sticky='W')


#Image
im_path = os.path.join(scriptDir, 'Interface Images', 'Tab 2 Image.png')
image = Image.open(im_path)

photo = ImageTk.PhotoImage(image)
im_label = ttk.Label(tab2, image=photo)
im_label.grid(column=2, columnspan=3, row=0, rowspan=7, padx=15, pady=8, sticky='ne')
