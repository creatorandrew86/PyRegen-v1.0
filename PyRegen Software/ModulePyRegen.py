...
def PyRegen_func(PyRegen_args, c: CEA_Obj):
    ...
    if not abs(i_initial - i_final) == len(x_points):
        print("WARNING: The number of stations you picked covers the whole engine! Only the stations at which the analysis took place are"
        " shown here!!! \n")
    ...
    for it in range(abs(i_initial - i_final)):
        ...
        # Print to console
        print(f'Station: {it}, \u03B5 = {round(eps_i, 2)}, Tl(K) = {round(coolant_T, 2)}, Pl(bar) = {round(coolant_P_show, 2)}, T_wg(K) = {round(T_wg[0], 2)}, hg(kW/m^2K) = {round(hg[0]/1000, 2)}, hl(kW/m^2K) = {round(hl/1000, 2)}, Q_flux(MW/m^2) = {round(Q_flux/pow(10, 6), 2)}, coolant_Re = {round(coolant_Re, 2)}')
