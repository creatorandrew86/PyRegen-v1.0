...
def Inputk(event = None):
    ...
    if mat_combobox.get() == 'Custom':
        ttk.Label(tab3, text='Thermal Conductivity (W/mK):').grid(column=3, row=1, padx=5, pady=5, sticky='W')
...
def mainFunction():
    ...
    inputs_list, input_error = ModuleProcessInputs.process_inputs(entries_list)
    ...
    # Print initial values to console
    print(f"Chamber / Nozzle Initial Values : Oxidizer : {Ox}, Fuel : {Fuel}, Pc(bar) = {round(Pc / 14.5, 1)}, Area ratio = {eps}, Rt(cm) = {Rt}, Tc(K) = {round(Tc, 1)}")
    print(f"Coolant Initial Values : T(K) = {coolant_T}, P(bar) = {round(coolant_P / 101325, 2)}, Mass Flow (kg/s) = {coolant_mfr} \n")
    print("-------------------------------------------------------------------------------------------------")
...
def execute_PyRegen():
    ...
    # Print console initializer
    print(f"PyRegen Run {run_counter} \n")
    print("------------------------------------------------------------------------------------------------- \n")
...
ttk.Label(tab3, text='Check the README file in the Github repository on how to fill the Channel Geometry entries!').grid(
    column=0, columnspan=5, row=10, padx=5, pady=15, sticky='sw'
)
...
mat_options = ['Copper', 'GRCop 42', 'GRCop 84', 'Custom']
