...
def process_inputs(entries_list):
    ...
    # ---------------------------------------------------------------------------------------------------------------------------- #
    # Handle inputs with no unit (MR, CR, eps)
    ...
    if number_it.get().strip():
        it_number = float(number_it.get())
    else:
        print("Fill the Number of Iterations Entry - Tab 2")
        input_error= True
    ...
    output_list = [FuelTemp, OxTemp, Pc, MR, CR, eps, it_number, charLength, Rt, 
                   eng_mass_flux, coolant_P, coolant_T, mfr, x_initial, x_final, input_error]
    return output_list
